...
class METS_SLA_Monitor {
	...
	public function add_cron_intervals( $schedules ) {
		// Add 15-minute interval for SLA monitoring
		$schedules['mets_sla_check_interval'] = array(
			'interval' => 900, // 15 minutes
			'display'  => __( 'Every 15 minutes (SLA Monitoring)', METS_TEXT_DOMAIN ),
		);

		// Add 5-minute interval for urgent SLA monitoring
		$schedules['mets_urgent_sla_check'] = array(
			'interval' => 300, // 5 minutes
			'display'  => __( 'Every 5 minutes (Urgent SLA)', METS_TEXT_DOMAIN ),
		);
	...
	private function check_response_sla( $ticket, $sla_rule, $current_time ) {
		...
		$warning_threshold = $response_due - ( $sla_rule->response_time * 3600 * 0.2 );
	...
	private function check_resolution_sla( $ticket, $sla_rule, $current_time ) {
		...
		// Calculate warning time (80% of SLA time)
		$warning_threshold = $resolution_due - ( $sla_rule->resolution_time * 3600 * 0.2 );
	...
	private function update_ticket_sla_dates( $ticket, $sla_rule ) {
		...
		if ( ! empty( $sla_rule->response_time ) ) {
		...
		if ( ! empty( $sla_rule->resolution_time ) ) {
