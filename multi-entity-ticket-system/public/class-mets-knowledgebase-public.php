...
class METS_Knowledgebase_Public {
	...
	public function enqueue_scripts() {
		...
		wp_localize_script( $this->plugin_name . '-kb-public', 'mets_kb_ajax', array(
			...
			'strings' => array(
				'search_placeholder' => __( 'Search knowledgebase...', METS_TEXT_DOMAIN ),
				'no_results' => __( 'No articles found.', METS_TEXT_DOMAIN ),
				'loading' => __( 'Loading...', METS_TEXT_DOMAIN ),
				'helpful_yes' => __( 'Yes', METS_TEXT_DOMAIN ),
				'helpful_no' => __( 'No', METS_TEXT_DOMAIN ),
				'thank_you' => __( 'Thank you for your feedback!', METS_TEXT_DOMAIN ),
	...
	public function display_kb_search_shortcode( $atts ) {
		$atts = shortcode_atts( array(
			'placeholder' => __( 'Search knowledgebase...', METS_TEXT_DOMAIN ),
			'entity_id' => null,
	...
	private function display_kb_home() {
		...
		<div class="mets-kb-home">
			<header class="mets-kb-header">
				<h1><?php _e( 'Knowledge Base', METS_TEXT_DOMAIN ); ?></h1>
				<p class="mets-kb-description"><?php _e( 'Find answers to your questions and browse our help articles.', METS_TEXT_DOMAIN ); ?></p>
			</header>
			...
			<section class="mets-kb-entity-filter">
				<h3><?php _e( 'Browse by Service Area', METS_TEXT_DOMAIN ); ?></h3>
				<div class="mets-entity-grid">
					<a href="<?php echo home_url( '/knowledgebase/' ); ?>" class="mets-entity-card <?php echo ! $current_entity_id ? 'active' : ''; ?>">
						...
						<h4><?php _e( 'All Areas', METS_TEXT_DOMAIN ); ?></h4>
						<p><?php _e( 'Browse all available content', METS_TEXT_DOMAIN ); ?></p>
					</a>
			...
			<section class="mets-kb-recent-section">
				<h3><?php _e( 'Recently Added', METS_TEXT_DOMAIN ); ?></h3>
	...
	private function display_kb_search_results() {
		...
		<div class="mets-kb-search-page">
			<header class="mets-kb-header">
				<h1><?php _e( 'Search Results', METS_TEXT_DOMAIN ); ?></h1>
				<?php if ( ! empty( $search_query ) ): ?>
					<p class="mets-kb-search-query"><?php printf( __( 'Results for: "%s"', METS_TEXT_DOMAIN ), esc_html( $search_query ) ); ?></p>
				<?php endif; ?>
			</header>
		...
		<div class="mets-kb-category-page">
			<header class="mets-kb-header">
				<nav class="mets-kb-breadcrumb">
					<a href="<?php echo home_url( '/knowledgebase/' ); ?>"><?php _e( 'Knowledge Base', METS_TEXT_DOMAIN ); ?></a>
	...
	private function render_search_form( $atts ) {
		$placeholder = isset( $atts['placeholder'] ) ? $atts['placeholder'] : __( 'Search knowledgebase...', METS_TEXT_DOMAIN );
			...
			<form action="<?php echo home_url( '/knowledgebase/search/' ); ?>" method="get" id="mets-kb-search">
				<div class="mets-search-input-wrapper">
					...
					<button type="submit" class="mets-search-submit">
						...
						<span class="screen-reader-text"><?php _e( 'Search', METS_TEXT_DOMAIN ); ?></span>
					</button>
				</div>
				<?php if ( $entity_id ): ?>
	...
	private function display_404() {
		...
		<div class="mets-kb-404">
			<h1><?php _e( 'Article Not Found', METS_TEXT_DOMAIN ); ?></h1>
			<p><?php _e( 'The article you are looking for could not be found.', METS_TEXT_DOMAIN ); ?></p>
			<a href="<?php echo home_url( '/knowledgebase/' ); ?>" class="mets-kb-button">
				<?php _e( 'Back to Knowledge Base', METS_TEXT_DOMAIN ); ?>
			</a>
		</div>
		<?php
	}
	...
	private function display_access_denied() {
		...
		<div class="mets-kb-access-denied">
			<h1><?php _e( 'Access Denied', METS_TEXT_DOMAIN ); ?></h1>
			<p><?php _e( 'You do not have permission to view this content.', METS_TEXT_DOMAIN ); ?></p>
			<a href="<?php echo home_url( '/knowledgebase/' ); ?>" class="mets-kb-button">
				<?php _e( 'Back to Knowledge Base', METS_TEXT_DOMAIN ); ?>
			</a>
		</div>
	...
	private function render_categories_widget( $atts ) {
		...
		if ( empty( $categories ) ) {
			echo '<p class="mets-kb-no-content">' . __( 'No categories found.', METS_TEXT_DOMAIN ) . '</p>';
			return;
		}

		?>
		<h3><?php _e( 'Browse by Category', METS_TEXT_DOMAIN ); ?></h3>
			...
			<?php foreach ( $categories as $category ): ?>
				<a href="<?php echo home_url( '/knowledgebase/category/' . $category->slug . '/' ); ?>" class="mets-kb-category-card">
					...
					<div class="mets-kb-category-content">
						...
						<?php endif; ?>
						<?php if ( $show_count ): ?>
							<span class="mets-kb-category-count"><?php printf( _n( '%s article', '%s articles', $category->article_count, METS_TEXT_DOMAIN ), number_format( $category->article_count ) ); ?></span>
						<?php endif; ?>
					</div>
				</a>
			<?php endforeach; ?>
	...
	private function render_popular_articles( $atts ) {
		...
		if ( empty( $articles ) ) {
			echo '<p class="mets-kb-no-content">' . __( 'No articles found.', METS_TEXT_DOMAIN ) . '</p>';
			return;
		}
					...
					<div class="mets-kb-article-meta">
						<?php if ( $show_views ): ?>
							<span class="mets-kb-views">
								...
								<?php echo number_format( $article->view_count ); ?> <?php _e( 'views', METS_TEXT_DOMAIN ); ?>
							</span>
						<?php endif; ?>
						<?php if ( $show_rating && $article->helpful_yes + $article->helpful_no > 0 ): ?>
							<span class="mets-kb-rating">
								...
								<?php echo $rating_percentage; ?>% <?php _e( 'helpful', METS_TEXT_DOMAIN ); ?>
							</span>
						<?php endif; ?>
					</div>
				</article>
	...
	private function render_recent_articles( $atts ) {
		...
		if ( empty( $articles ) ) {
			echo '<p class="mets-kb-no-content">' . __( 'No recent articles found.', METS_TEXT_DOMAIN ) . '</p>';
			return;
		}
			...
			<?php foreach ( $articles as $article ): ?>
				<article class="mets-kb-article-card">
					...
					<?php if ( $article->is_featured ): ?>
						<span class="mets-kb-featured-badge"><?php _e( 'Featured', METS_TEXT_DOMAIN ); ?></span>
					<?php endif; ?>
				</article>
			<?php endforeach; ?>
		</div>
		...
		<div class="mets-kb-search-results">
			<?php if ( ! empty( $search_query ) ): ?>
				<div class="mets-kb-results-header">
					<p class="mets-kb-results-count">
						<?php printf( _n( '%s result found', '%s results found', $total, METS_TEXT_DOMAIN ), number_format( $total ) ); ?>
					</p>
				</div>
			<?php endif; ?>
					...
					<?php foreach ( $articles as $article ): ?>
						<article class="mets-kb-search-result">
							...
							<div class="mets-kb-search-meta">
								...
								<?php endif; ?>
								<span class="mets-kb-article-views"><?php echo number_format( $article->view_count ); ?> <?php _e( 'views', METS_TEXT_DOMAIN ); ?></span>
			...
			<?php else: ?>
				<div class="mets-kb-no-results">
					<h3><?php _e( 'No results found', METS_TEXT_DOMAIN ); ?></h3>
					<p><?php _e( 'Try adjusting your search terms or browse our categories below.', METS_TEXT_DOMAIN ); ?></p>
	...
	private function render_article_content( $article ) {
		...
		<article class="mets-kb-article">
			<header class="mets-kb-article-header">
				...
				<div class="mets-kb-article-meta">
					<time><?php echo date_i18n( get_option( 'date_format' ), strtotime( $article->created_at ) ); ?></time>
					<span class="mets-kb-views"><?php echo number_format( $article->view_count ); ?> <?php _e( 'views', METS_TEXT_DOMAIN ); ?></span>
					<?php if ( $article->entity_name ): ?>
			...
			<?php if ( ! empty( $attachments ) ): ?>
				<section class="mets-kb-article-attachments">
					<h3><?php _e( 'Downloads', METS_TEXT_DOMAIN ); ?></h3>
			...
			<?php if ( ! empty( $tags ) ): ?>
				<section class="mets-kb-article-tags">
					<h4><?php _e( 'Tags:', METS_TEXT_DOMAIN ); ?></h4>
	...
	private function render_article_feedback( $article ) {
		...
		<section class="mets-kb-article-feedback">
			<div class="mets-kb-feedback-header">
				<h3><?php _e( 'Was this article helpful?', METS_TEXT_DOMAIN ); ?></h3>
				<?php if ( $total_votes > 0 ): ?>
					<p class="mets-kb-feedback-stats">
						<?php printf( __( '%d%% of users found this helpful (%d votes)', METS_TEXT_DOMAIN ), $helpful_percentage, $total_votes ); ?>
					</p>
				<?php endif; ?>
			</div>
		...
		<div class="mets-kb-pagination">
			<?php if ( $current_page > 1 ): ?>
				<a href="<?php echo esc_url( add_query_arg( array_merge( $args, array( 'paged' => $current_page - 1 ) ), $base_url ) ); ?>" class="mets-kb-page-btn mets-kb-prev">
					&laquo; <?php _e( 'Previous', METS_TEXT_DOMAIN ); ?>
				</a>
			<?php endif; ?>
			...
			<?php if ( $current_page < $total_pages ): ?>
				<a href="<?php echo esc_url( add_query_arg( array_merge( $args, array( 'paged' => $current_page + 1 ) ), $base_url ) ); ?>" class="mets-kb-page-btn mets-kb-next">
					<?php _e( 'Next', METS_TEXT_DOMAIN ); ?> &raquo;
				</a>
			<?php endif; ?>
		</div>
		<?php
	...
	public function handle_article_rating() {
		...
		if ( ! in_array( $vote, array( 'yes', 'no' ) ) ) {
			wp_send_json_error( __( 'Invalid vote.', METS_TEXT_DOMAIN ) );
		}
		...
		if ( $result ) {
			...
			wp_send_json_error( __( 'Failed to record your vote.', METS_TEXT_DOMAIN ) );
		}
	}
	...
	public function handle_attachment_download() {
		...
		if ( ! $attachment ) {
			wp_die( __( 'Attachment not found.', METS_TEXT_DOMAIN ) );
		}
		...
		if ( ! $article || ! $this->can_view_article( $article ) ) {
			wp_die( __( 'Access denied.', METS_TEXT_DOMAIN ) );
		}
		...
		if ( file_exists( $attachment->file_path ) ) {
			...
			wp_die( __( 'File not found.', METS_TEXT_DOMAIN ) );
		}
	}
	...
	private function render_category_articles( $category_id ) {
		...
		if ( empty( $articles ) ) {
			echo '<p class="mets-kb-no-content">' . __( 'No articles found in this category.', METS_TEXT_DOMAIN ) . '</p>';
			return;
		}
	...
	private function render_related_articles( $article ) {
		...
		<section class="mets-kb-related-articles">
			<h3><?php _e( 'Related Articles', METS_TEXT_DOMAIN ); ?></h3>
		...
		<div class="mets-kb-tag-page">
			<header class="mets-kb-header">
				<nav class="mets-kb-breadcrumb">
					<a href="<?php echo home_url( '/knowledgebase/' ); ?>"><?php _e( 'Knowledge Base', METS_TEXT_DOMAIN ); ?></a>
					<span class="separator">&gt;</span>
					<span class="current"><?php printf( __( 'Tag: %s', METS_TEXT_DOMAIN ), esc_html( $tag->name ) ); ?></span>
				</nav>
				
				<h1><?php printf( __( 'Articles tagged "%s"', METS_TEXT_DOMAIN ), esc_html( $tag->name ) ); ?></h1>
			...
			<div class="mets-kb-tag-content">
				<?php if ( ! empty( $results['articles'] ) ): ?>
					...
					<?php endif; ?>
				<?php else: ?>
					<p class="mets-kb-no-content"><?php _e( 'No articles found with this tag.', METS_TEXT_DOMAIN ); ?></p>
				<?php endif; ?>
			</div>
		</div>
		<?php
