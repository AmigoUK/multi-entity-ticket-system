...
class METS_Custom_Report_Builder {
	...
	private function generate_ticket_report( $config ) {
		...
		LEFT JOIN {$wpdb->prefix}users u ON t.created_by = u.ID
	...
	private function build_date_where( $config ) {
		...
		switch ( $date_range ) {
			case 'today':
				return "DATE(analytics.created_at) = CURDATE()";
			case 'yesterday':
				return "DATE(analytics.created_at) = DATE_SUB(CURDATE(), INTERVAL 1 DAY)";
			case 'last_7_days':
				return "analytics.created_at >= DATE_SUB(NOW(), INTERVAL 7 DAY)";
			case 'last_30_days':
				return "analytics.created_at >= DATE_SUB(NOW(), INTERVAL 30 DAY)";
			case 'last_90_days':
				return "analytics.created_at >= DATE_SUB(NOW(), INTERVAL 90 DAY)";
			case 'custom':
				$date_from = sanitize_text_field( $config['date_from'] ?? '' );
				$date_to = sanitize_text_field( $config['date_to'] ?? '' );
				if ( $date_from && $date_to ) {
					return "DATE(analytics.created_at) BETWEEN '{$date_from}' AND '{$date_to}'";
				}
				break;
		}
