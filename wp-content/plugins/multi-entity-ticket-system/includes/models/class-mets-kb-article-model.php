...
class METS_KB_Article_Model {
	...
	public function create( $data ) {
		...
		$article_id = $result !== false ? $wpdb->insert_id : false;
		
		// Clear search cache when new article is created
		if ( $article_id ) {
	...
	public function update( $id, $data ) {
		...
		// Clear search cache when article is updated
		if ( $success ) {
			$this->clear_search_cache();
		}
	...
	/**
	 * Get articles with inheritance (entity + parent articles) with caching support
	 *
	 * @since    1.0.0
	...
	public function get_articles_with_inheritance( $args = array() ) {
		...
		if ( ! empty( $args['search'] ) ) {
			$cache_key = 'mets_kb_search_' . md5( serialize( $args ) );
			$cached_result = wp_cache_get( $cache_key, 'mets_kb_search' );
		...
		// Search filter with FULLTEXT optimization for better performance
		$search_relevance_select = '';
		$use_fulltext = false;
		if ( ! empty( $args['search'] ) ) {
			$search_term = $wpdb->esc_like( $args['search'] );
			
			// Use FULLTEXT search for better performance and relevance scoring
			if ( strlen( $search_term ) >= 3 ) {
				// FULLTEXT search with relevance scoring
				$where_conditions[] = "MATCH(a.title, a.content, a.excerpt) AGAINST(%s IN NATURAL LANGUAGE MODE)";
				$search_relevance_select = ", MATCH(a.title, a.content, a.excerpt) AGAINST(%s IN NATURAL LANGUAGE MODE) as search_relevance";
				$params[] = $search_term; // For WHERE clause
				$use_fulltext = true;
			} else {
		...
		// Build ORDER BY clause with search relevance optimization
		$allowed_orderby = array( 'title', 'created_at', 'updated_at', 'view_count', 'sort_order', 'featured', 'relevance' );
		$orderby = in_array( $args['orderby'], $allowed_orderby ) ? $args['orderby'] : 'created_at';
		$order = strtoupper( $args['order'] ) === 'ASC' ? 'ASC' : 'DESC';
		
		// Use relevance ordering for search queries
		if ( ! empty( $args['search'] ) && $use_fulltext ) {
			// FULLTEXT search with relevance scoring
			$order_clause = "ORDER BY search_relevance DESC, a.featured DESC, a.view_count DESC";
		} elseif ( ! empty( $args['search'] ) ) {
			// LIKE search fallback - order by featured and view count  
			$order_clause = "ORDER BY a.featured DESC, a.view_count DESC, a.created_at DESC";
		} elseif ( $orderby === 'relevance' ) {
			// Fallback to view_count for relevance when no search term (popularity-based relevance)
			$order_clause = "ORDER BY a.view_count DESC, a.featured DESC, a.created_at DESC";
		...
		// Main query with search relevance
		$sql = "SELECT a.*, u.display_name as author_name, 
		               ur.display_name as reviewer_name,
		               e.name as entity_name{$search_relevance_select}
		        FROM {$this->table_name} a
		        LEFT JOIN {$wpdb->prefix}users u ON a.author_id = u.ID
		        LEFT JOIN {$wpdb->prefix}users ur ON a.reviewed_by = ur.ID
		        LEFT JOIN {$wpdb->prefix}mets_entities e ON a.entity_id = e.id
		...
		// Handle parameters for FULLTEXT search
		$query_params = $params;
		if ( $use_fulltext ) {
			// Add search term again for SELECT relevance clause
			array_splice( $query_params, 0, 0, $search_term );
		}
		
		$articles = $wpdb->get_results( $wpdb->prepare( $sql, $query_params ) );
		...
		if ( ! empty( $cache_key ) ) {
			wp_cache_set( $cache_key, $result, 'mets_kb_search', 300 );
		}
	...
	public function delete( $id ) {
		...
		// Clear search cache when article is deleted
		if ( $success ) {
			$this->clear_search_cache();
		}
	...
	public function count_articles_by_category( $category_id ) {
		...
		$sql = "SELECT COUNT(*) 
		        FROM {$this->table_name} a
		        INNER JOIN {$wpdb->prefix}mets_kb_article_categories ac ON a.id = ac.article_id
		        WHERE ac.category_id = %d";
		
		return intval( $wpdb->get_var( $wpdb->prepare( $sql, $category_id ) ) );
	}
	...
	private function clear_search_cache() {
		wp_cache_flush_group( 'mets_kb_search' );
	}
}