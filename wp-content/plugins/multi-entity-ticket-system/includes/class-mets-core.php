...
class METS_Core {
	...
	public function __construct() {
		...
		if ( ! self::$hooks_registered ) {
			...
			// Re-enabling advanced features gradually  
			$this->initialize_advanced_automation();
			$this->initialize_performance_optimization();
			$this->initialize_security_system(); // Re-enabled with safe configuration
			$this->initialize_csp_handler(); // Re-enabled with improved compatibility
			$this->initialize_ajax_polling(); // New AJAX polling system
			// AI integration delayed until after textdomain loading
			$this->loader->add_action( 'init', $this, 'initialize_ai_integration', 15 );
			// $this->initialize_realtime_features(); // Disabled - WebSocket features removed
	...
	private function load_dependencies() {
		...
		/**
		 * Enhanced KB Article Management System
		 */
		if ( is_admin() ) {
			require_once METS_PLUGIN_PATH . 'admin/kb/class-mets-kb-admin-integration.php';
		}
		...
		require_once METS_PLUGIN_PATH . 'includes/class-mets-performance-monitor.php';
	...
	public function add_cron_schedules( $schedules ) {
		// Add general intervals - use non-translated strings to avoid early translation loading
		$schedules['five_minutes'] = array(
			'interval' => 300,
			'display'  => did_action( 'init' ) ? __( 'Every 5 Minutes', METS_TEXT_DOMAIN ) : 'Every 5 Minutes',
		);

		$schedules['fifteen_minutes'] = array(
			'interval' => 900,
			'display'  => did_action( 'init' ) ? __( 'Every 15 Minutes', METS_TEXT_DOMAIN ) : 'Every 15 Minutes',
		);

		$schedules['thirty_minutes'] = array(
			'interval' => 1800,
			'display'  => did_action( 'init' ) ? __( 'Every 30 Minutes', METS_TEXT_DOMAIN ) : 'Every 30 Minutes',
		);

		$schedules['two_hours'] = array(
			'interval' => 7200,
			'display'  => did_action( 'init' ) ? __( 'Every 2 Hours', METS_TEXT_DOMAIN ) : 'Every 2 Hours',
		);
	...
	private function initialize_performance_optimization() {
		...
		$performance_monitor = METS_Performance_Monitor::get_instance();
		...
		$this->loader->add_action( 'mets_performance_cleanup', $performance_monitor, 'cleanup_old_performance_data' );
		
		// Initialize performance schedules
		$this->loader->add_action( 'init', $this, 'initialize_performance_schedules' );
		
		// Enable performance monitoring if debug is enabled or explicitly set
		if ( defined( 'WP_DEBUG' ) && WP_DEBUG || defined( 'METS_PERFORMANCE_MONITORING' ) && METS_PERFORMANCE_MONITORING ) {
			if ( ! defined( 'METS_PERFORMANCE_MONITORING' ) ) {
	...
	public function initialize_performance_schedules() {
		...
		if ( ! get_option( 'mets_enhanced_indexes_created' ) ) {
			$database_optimizer = METS_Database_Optimizer::get_instance();
			$database_optimizer->create_enhanced_composite_indexes();
		}
	}
	...
	public function ajax_clear_security_logs() {
		...
		$message = did_action( 'init' ) ? 
			sprintf( __( 'Deleted %d old security log entries', METS_TEXT_DOMAIN ), $deleted ) :
			sprintf( 'Deleted %d old security log entries', $deleted );
	...
	public function ajax_reset_rate_limits() {
		...
		$message = did_action( 'init' ) ? 
			sprintf( __( 'Reset %d rate limit entries', METS_TEXT_DOMAIN ), $deleted ) :
			sprintf( 'Reset %d rate limit entries', $deleted );
	 ...
	 * @access   public
	 */
	public function initialize_ai_integration() {
		...
		if ( $ai_service->is_configured() ) {
			require_once METS_PLUGIN_PATH . 'includes/class-mets-ai-integration.php';
			METS_AI_Integration::get_instance();
		}
		
		// Initialize AI Chat Widget (independent of AI service status)
		if ( file_exists( METS_PLUGIN_PATH . 'public/class-mets-ai-chat-widget.php' ) ) {
			require_once METS_PLUGIN_PATH . 'public/class-mets-ai-chat-widget.php';
			
			// Ensure the widget initialization function runs
	...
	/**
	 * Initialize AJAX polling system
	 *
	 * @since    1.0.0
	 * @access   private
	 */
	private function initialize_ajax_polling() {
		require_once METS_PLUGIN_PATH . 'includes/class-mets-ajax-polling.php';
		METS_Ajax_Polling::get_instance();
		
		// Initialize admin bar integration
		require_once METS_PLUGIN_PATH . 'includes/class-mets-admin-bar.php';
		METS_Admin_Bar::get_instance();
		
		// Enqueue notification styles
		$this->loader->add_action( 'admin_enqueue_scripts', $this, 'enqueue_notification_styles' );
	}
	 ...
	 * @since    1.0.0
	 */
	public function enqueue_notification_styles() {
		wp_enqueue_style(
			'mets-notifications',
			METS_PLUGIN_URL . 'assets/css/mets-notifications.css',
			array(),
			METS_VERSION,
			'all'
		);
	 ...
	 * @access   private
	 */
	private function initialize_realtime_features() {
		// Disabled - WebSocket features removed
		// add_action( 'init', array( $this, 'init_websocket_integration' ), 10 );
		
		// Disabled - real-time CSS removed
		// $this->loader->add_action( 'admin_enqueue_scripts', $this, 'enqueue_realtime_styles' );
	}
	
	// Removed - WebSocket features deleted
	/*
	 * Initialize WebSocket integration after WordPress is loaded
	 *
	public function init_websocket_integration() {
		// Initialize WebSocket integration
		require_once METS_PLUGIN_PATH . 'includes/class-mets-websocket-integration.php';
		METS_WebSocket_Integration::get_instance();
	}*/
	
	// Removed - WebSocket features deleted
	/*
	 * Enqueue real-time styles
	 *
	 * @since    1.0.0
	 *
	public function enqueue_realtime_styles() {
		wp_enqueue_style(
			'mets-realtime',
			METS_PLUGIN_URL . 'assets/css/mets-realtime.css',
			array(),
			METS_VERSION
		);
	}*/
	...
	private function initialize_knowledge_base() {
		// KB models are loaded in load_dependencies()
