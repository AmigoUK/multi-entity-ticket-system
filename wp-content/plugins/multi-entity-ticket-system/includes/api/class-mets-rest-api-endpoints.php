...
class METS_REST_API_Endpoints extends METS_REST_API {
	...
	private function prepare_ticket_for_response( $ticket, $detailed = false ) {
		$data = array(
			...
			'customer_id' => $ticket->created_by ? intval( $ticket->created_by ) : null,
	...
	private function prepare_sla_rule_for_response( $rule ) {
		return array(
			...
			'response_time' => intval( $rule->response_time_hours ),
			'resolution_time' => intval( $rule->resolution_time_hours ),
	...
	private function get_date_where_clause( $period ) {
		switch ( $period ) {
			case '24hours':
				return " AND an.created_at >= DATE_SUB(NOW(), INTERVAL 24 HOUR)";
			case '7days':
				return " AND an.created_at >= DATE_SUB(NOW(), INTERVAL 7 DAY)";
			case '30days':
				return " AND an.created_at >= DATE_SUB(NOW(), INTERVAL 30 DAY)";
			case '90days':
				return " AND an.created_at >= DATE_SUB(NOW(), INTERVAL 90 DAY)";
			case 'all':
			default:
				return "";
		}
