...
class METS_Agent_Status_Widget {
	...
	public function add_admin_bar_status( $wp_admin_bar ) {
		...
		$wp_admin_bar->add_node( array(
			...
			'meta' => array(
				'title' => $status_message ?: ( did_action( 'init' ) ? __( 'Click to update your status', METS_TEXT_DOMAIN ) : 'Click to update your status' )
			)
		) );
		
		// Add status options
		$statuses = array(
			'available' => did_action( 'init' ) ? __( 'Available', METS_TEXT_DOMAIN ) : 'Available',
			'busy' => did_action( 'init' ) ? __( 'Busy', METS_TEXT_DOMAIN ) : 'Busy',
			'away' => did_action( 'init' ) ? __( 'Away', METS_TEXT_DOMAIN ) : 'Away',
			'offline' => did_action( 'init' ) ? __( 'Offline', METS_TEXT_DOMAIN ) : 'Offline'
		);
		...
		$wp_admin_bar->add_node( array(
			'id' => 'mets-my-tickets',
			'parent' => 'mets-agent-status',
			'title' => sprintf( did_action( 'init' ) ? __( 'My Tickets (%d)', METS_TEXT_DOMAIN ) : 'My Tickets (%d)', $workload ),
		...
		$wp_admin_bar->add_node( array(
			...
			'parent' => 'mets-agent-status',
			'title' => did_action( 'init' ) ? __( 'Agent Profile', METS_TEXT_DOMAIN ) : 'Agent Profile',
			'href' => admin_url( 'profile.php#mets-agent-profile' )
		) );
	...
	public function add_dashboard_widget() {
		...
		wp_add_dashboard_widget(
			...
			did_action( 'init' ) ? __( 'My Agent Status', METS_TEXT_DOMAIN ) : 'My Agent Status',
	...
	public function render_dashboard_widget() {
		...
		<div class="mets-agent-status-widget">
			...
			<div class="mets-status-section">
				<h4><?php echo did_action( 'init' ) ? __( 'Current Status', METS_TEXT_DOMAIN ) : 'Current Status'; ?></h4>
				<div class="mets-status-selector">
					<select id="mets-dashboard-status" style="width: 100%;">
						<option value="available" <?php selected( $preferences['status'], 'available' ); ?>><?php echo did_action( 'init' ) ? __( 'Available', METS_TEXT_DOMAIN ) : 'Available'; ?></option>
						<option value="busy" <?php selected( $preferences['status'], 'busy' ); ?>><?php echo did_action( 'init' ) ? __( 'Busy', METS_TEXT_DOMAIN ) : 'Busy'; ?></option>
						<option value="away" <?php selected( $preferences['status'], 'away' ); ?>><?php echo did_action( 'init' ) ? __( 'Away', METS_TEXT_DOMAIN ) : 'Away'; ?></option>
						<option value="offline" <?php selected( $preferences['status'], 'offline' ); ?>><?php echo did_action( 'init' ) ? __( 'Offline', METS_TEXT_DOMAIN ) : 'Offline'; ?></option>
					</select>
					<input type="text" id="mets-status-message" placeholder="<?php echo esc_attr( did_action( 'init' ) ? __( 'Status message (optional)', METS_TEXT_DOMAIN ) : 'Status message (optional)' ); ?>" value="<?php echo esc_attr( $preferences['status_message'] ); ?>" style="width: 100%; margin-top: 5px;" />
					<button class="button button-primary" id="mets-update-status" style="margin-top: 5px;"><?php echo did_action( 'init' ) ? __( 'Update Status', METS_TEXT_DOMAIN ) : 'Update Status'; ?></button>
				</div>
			</div>
			
			<!-- Workload Info -->
			<div class="mets-workload-section" style="margin-top: 20px;">
				<h4><?php echo did_action( 'init' ) ? __( 'Workload', METS_TEXT_DOMAIN ) : 'Workload'; ?></h4>
				<div class="mets-workload-info">
					<div class="mets-stat">
						<span class="mets-stat-label"><?php echo did_action( 'init' ) ? __( 'Active Tickets:', METS_TEXT_DOMAIN ) : 'Active Tickets:'; ?></span>
			...
			<div class="mets-performance-section" style="margin-top: 20px;">
				<h4><?php echo did_action( 'init' ) ? __( 'This Week', METS_TEXT_DOMAIN ) : 'This Week'; ?></h4>
				<div class="mets-stat-grid">
					<div class="mets-stat">
						<span class="mets-stat-label"><?php echo did_action( 'init' ) ? __( 'Resolved:', METS_TEXT_DOMAIN ) : 'Resolved:'; ?></span>
						<span class="mets-stat-value"><?php echo esc_html( $performance['resolved_tickets'] ); ?></span>
					</div>
					<div class="mets-stat">
						<span class="mets-stat-label"><?php echo did_action( 'init' ) ? __( 'Avg Response:', METS_TEXT_DOMAIN ) : 'Avg Response:'; ?></span>
			...
			<div class="mets-quick-links" style="margin-top: 20px; padding-top: 20px; border-top: 1px solid #dcdcde;">
				<a href="<?php echo admin_url( 'admin.php?page=mets-all-tickets&assigned_to=' . $current_user_id ); ?>" class="button"><?php echo did_action( 'init' ) ? __( 'My Tickets', METS_TEXT_DOMAIN ) : 'My Tickets'; ?></a>
				<a href="<?php echo admin_url( 'profile.php#mets-agent-profile' ); ?>" class="button"><?php echo did_action( 'init' ) ? __( 'Edit Profile', METS_TEXT_DOMAIN ) : 'Edit Profile'; ?></a>
			</div>
		</div>
	...
	public function ajax_quick_status_update() {
		...
		if ( ! in_array( $status, array( 'available', 'busy', 'away', 'offline' ) ) ) {
			wp_send_json_error( array( 'message' => did_action( 'init' ) ? __( 'Invalid status.', METS_TEXT_DOMAIN ) : 'Invalid status.' ) );
		}
		...
		wp_send_json_success( array( 
			'message' => did_action( 'init' ) ? __( 'Status updated successfully.', METS_TEXT_DOMAIN ) : 'Status updated successfully.',
			'status' => $status
		) );
	}
}