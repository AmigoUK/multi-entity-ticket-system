...
function run_multi_entity_ticket_system() {
	...
	// Load and initialize guest ticket access extension
	if ( file_exists( METS_PLUGIN_PATH . 'public/class-mets-guest-ticket-access.php' ) ) {
		require_once METS_PLUGIN_PATH . 'public/class-mets-guest-ticket-access.php';
		$guest_access = new METS_Guest_Ticket_Access();
		$guest_access->init();
	}
}
...
/**
 * Add admin menu for schema testing tool
 * Only visible to administrators
 * Moved to init hook to prevent early execution
 */
function mets_add_schema_testing_menu() {
    if (current_user_can('manage_options')) {
        add_management_page(
            ...
            'manage_options',
            'mets-schema-testing',
            'mets_schema_testing_page'
        );
    }
}

/**
 * Schema testing page callback
 */
function mets_schema_testing_page() {
    // Include the schema testing tool
    include_once METS_PLUGIN_PATH . 'test-schema-improvements.php';
}

// Hook to init instead of admin_menu to prevent early execution
add_action('init', function() {
    if (is_admin()) {
        add_action('admin_menu', 'mets_add_schema_testing_menu');
    }
});
?>